...
@HighFailureBuilds = uniq(@HighFailureBuilds);
my $outfile;
my $scal = "";

open $outfile, '>>', "HighFailure_builds.txt" or die "Could not create file.\n";
$scal = join(",", @HighFailure);
print {$outfile} $scal;
close $outfile; 
#
#$select->finish();
